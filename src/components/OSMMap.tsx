...
export default function OSMMap({
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/geocode/search?q=${encodeURIComponent(searchQuery)}`
      );
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl border border-neutral-100 shadow-sm overflow-hidden">
      ...
      <div className="p-3 border-b border-neutral-100 bg-neutral-50/50 relative z-[1000]">
        ...
        {searchResults.length > 0 && (
          <div id="search-results-dropdown" className="absolute left-3 right-3 top-[3.5rem] z-[1000] max-h-56 overflow-y-auto bg-white border border-neutral-200 rounded-xl shadow-lg divide-y divide-neutral-100">
